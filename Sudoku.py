...
import time
import sys
from SudokuLogic import solve_sudoku

# Load OCR data for training
samples = np.float32(np.loadtxt('generalsamples.data'))
responses = np.float32(np.loadtxt('generalresponses.data'))
...
img = cv2.imread('C:\Users\Pallavi\Downloads\sudoku.jpg')
...
# Now we get perfect square with size 450x450

warpg = cv2.cvtColor(warp, cv2.COLOR_BGR2GRAY)
# kept a gray-scale copy of warp for further use

cv2.imshow("Grayscale image",warpg)
cv2.waitKey(0)
cv2.destroyAllWindows()
...
# ans is the solved sudoku we get as a string

# Uncomment below two lines if you want to print solved 9x9 matrix sudoku on terminal, optional

l = [int(i) for i in ans]
# we make string ans to a list
ansarray = np.array(l,np.uint8).reshape((9,9))
# Now we make it into an array of sudoku
...
cv2.imshow('Final Solution', warp)
